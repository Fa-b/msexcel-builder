...
(function() {
  var CalcChain, ContentTypes, DocPropsApp, JSDateToExcel, JSZip, SharedStrings, Sheet, Style, Workbook, XlRels, XlWorkbook, baseXl, fs, tool, xml,
  ...
  Sheet = (function() {
    ...
    Sheet.prototype.find = function(str) {
      let ret = undefined;
      if(this.book.ss.cache.hasOwnProperty(str)) {
        let id = this.book.ss.cache[str];
        ret = [];
        for (i = l = 1, ref = this.rows; 1 <= ref ? l <= ref : l >= ref; i = 1 <= ref ? ++l : --l) {
          for (j = n = 1, ref1 = this.cols; 1 <= ref1 ? n <= ref1 : n >= ref1; j = 1 <= ref1 ? ++n : --n) {
            if(this.data[i][j].v == id)
    ...
    Sheet.prototype.append_below = function(row, sheet) {
      this.rows = this.rows + sheet.rows + (row - this.rows);


      // Workbook and Sheetname are inherited
      let k = 1;
      for (let i = l = row, ref = this.rows; row <= ref ? l <= ref : l >= ref; i = row <= ref ? ++l : --l) {
        this.data[i] = {};
        for (let j = n = 1, ref1 = this.cols; 1 <= ref1 ? n <= ref1 : n >= ref1; j = 1 <= ref1 ? ++n : --n) {
          this.data[i][j] = {
            v: 0
          };
          if (sheet.data.hasOwnProperty(k) &&
            sheet.data[k].hasOwnProperty(j) &&
            sheet.data[k][j].hasOwnProperty('init')) {
            this.set(j, i, sheet.data[k][j].init);
            if(sheet.data[k][j].init.hasOwnProperty('formula')) {
              let matches = [...sheet.data[k][j].init.formula.matchAll(/(([A-Z]+)(\d+))/g)];
              let res = sheet.data[k][j].init.formula;
              matches.forEach(match => {
                res = res.replace(match[1], (match[2] + (parseInt(match[3]) + (row-1))))
              });
              this.formula(j, i, res);
            }
          }
        }
      ...
      sheet.merges.forEach(merge => {
        this.merge({
          row: merge.from.row + row - 1,
          col: merge.from.col
        }, {
          row: merge.to.row + row - 1,
          col: merge.to.col
        });
      });
    };

    Sheet.prototype.append_right = function(col, sheet) {
      this.cols = this.cols + sheet.cols + (col - this.cols);
      ...
      sheet.merges.forEach(merge => {
        this.merge({
          row: merge.from.row,
          col: merge.from.col + col - 1
        }, {
          row: merge.to.row,
          col: merge.to.col + col - 1
        });
      });
    };
    ...
    Sheet.prototype.set = function(col, row, str) {
      var key;
      if (!this.data[row][col].hasOwnProperty('init')) {
        this.data[row][col].init = {};
      }
      ...
      if (str instanceof Date) {
        ...
        // default localized (FMT 14):
        this.numberFormat(col, row, 'm/d/yy');
  ...
  CalcChain = (function() {
    ...
    CalcChain.prototype.toxml = function() {
      var cc, cells, i, key, l, len, n, ref, ref1;
      ...
      for (l = 0, len = ref.length; l < len; l++) {
        key = ref[l];
        cells = this.cache[key];
        for (i = n = 0, ref1 = this.cells; 0 <= ref1 ? n <= ref1 : n >= ref1; i = 0 <= ref1 ? ++n : --n) {
          cc.ele('c', {
            r: '' + cells[i],
            i: '' + key
          });
  ...
  Style = (function() {
    ...
    Style.prototype.font2id = function(font) {
      ...
      font || (font = {});
      font.bold || (font.bold = '-');
      font.iter || (font.iter = font.italic || '-');
      font.sz || (font.sz = '11');
