...
(function() {
  ...
  Sheet = (function() {
    ...
    Sheet.prototype.find = function(str) {
      var i, id, j, l, n, ref, ref1, ret;
      ret = void 0;
      if (this.book.ss.cache.hasOwnProperty(str)) {
        id = this.book.ss.cache[str];
        ret = [];
        for (i = l = 1, ref = this.rows; 1 <= ref ? l <= ref : l >= ref; i = 1 <= ref ? ++l : --l) {
          for (j = n = 1, ref1 = this.cols; 1 <= ref1 ? n <= ref1 : n >= ref1; j = 1 <= ref1 ? ++n : --n) {
    ...
    Sheet.prototype.append_below = function(row, sheet) {
      var i, j, k, l, len, merge, n, p, ref, ref1, ref2, ref3, res, results;
      this.rows = this.rows + sheet.rows + (row - this.rows);
      k = 1;
      for (i = l = ref = row, ref1 = this.rows; ref <= ref1 ? l <= ref1 : l >= ref1; i = ref <= ref1 ? ++l : --l) {
        this.data[i] = {};
        for (j = n = 1, ref2 = this.cols; 1 <= ref2 ? n <= ref2 : n >= ref2; j = 1 <= ref2 ? ++n : --n) {
          this.data[i][j] = {
            v: 0
          };
          if (sheet.data.hasOwnProperty(k) && sheet.data[k].hasOwnProperty(j) && sheet.data[k][j].hasOwnProperty('init')) {
            this.set(j, i, sheet.data[k][j].init);
            if (sheet.data[k][j].init.hasOwnProperty('formula')) {
              res = sheet.data[k][j].init.formula.replace(/(([A-Z]+)(\d+))/g, (function(_this) {
                return function(match, p1, p2, p3) {
                  return p2 + ((parseInt(p3)) + (row - 1));
                };
              })(this));
      ...
      ref3 = sheet.merges;
      results = [];
      for (p = 0, len = ref3.length; p < len; p++) {
        merge = ref3[p];
        results.push(this.merge({
  ...
  JSDateToExcel = function(dt) {
    return Math.round(25569.0 + ((dt.getTime() - (dt.getTimezoneOffset() * 60 * 1000)) / (1000 * 60 * 60 * 24)));
  };
